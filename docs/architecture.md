...
`Runtime::compile` validates marked outputs, input storage bindings, allocation totals, and MatMul dimension limits. It snapshots nodes and output identities, creates a stream, and allocates one Buffer per operator result. The builder can then be destroyed. Inputs still borrow caller-owned memory; replacing a Tensor pointer after compilation does not rebind that snapshot. Update input contents in the existing allocation before executing again. All storage must belong to the declared CUDA device.
...
The executable owns intermediate and computed-output storage until destruction or move assignment. Marked input-only outputs remain borrowed. Output tensor references remain valid only while their executable storage lives; another execution overwrites prior results. `allocated_bytes()` counts requested computed-tensor bytes, excluding borrowed inputs and CUDA stream/driver overhead. No lifetime-based reuse, pool, fusion, or kernel registry is claimed yet.
